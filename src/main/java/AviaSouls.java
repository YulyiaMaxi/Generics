...
public class AviaSouls {
    ...
    public Ticket[] search(String from, String to) {
        ...
        Arrays.sort(result);// реализация сортировки по цене
        return result;
    }
    public Ticket [] searchAndSortByTime(String from, String to, Comparator<Ticket> comparator) {// кроме параметров добавляем компаратор
        Ticket[] result = new Ticket[0]; // массив для ответа
        for (Ticket ticket : tickets) { // перебираем все билеты
            if (ticket.getFrom().equals(from)) { // совпадает аэропорт вылета
                if (ticket.getTo().equals(to)) { // совпадает аэропорт прилёта
        ...
        Arrays.sort(result, comparator);// реализация сортировки по времени полета, добавляем сортировку именно по условиям компаратора
        return result;
    }
    }
